...
python "$COLLECTIVEAPI_PATH/msccl-tools/examples/mscclang/allreduce_a100_ring.py" 8 1 1 \
    > "$SCRIPT_DIR/allreduce_ring_8ranks.xml"

echo "Converting XML to Chakra ET representation..."
python "$COLLECTIVEAPI_PATH/chakra_converter/et_converter.py" \
    --input_filename "$SCRIPT_DIR/allreduce_ring_8ranks.xml" \
    --output_filename "$SCRIPT_DIR/allreduce_ring_8ranks"
...
if [ "$CONVERT_TO_JSON" = "y" ] || [ -z "$CONVERT_TO_JSON" ]; then
    ...
    for i in {0..7}; do
        chakra_jsonizer \
            --input_filename "$SCRIPT_DIR/allreduce_ring_8ranks.$i.et" \
            --output_filename "$SCRIPT_DIR/allreduce_ring_8ranks.$i.json"
    done
fi